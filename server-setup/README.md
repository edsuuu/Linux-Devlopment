...
```bash
# 1. provisiona o servidor (pergunta sim/não por item: PHP+Composer, Node, nginx, pnpm)
./install.sh

# 2. cria a estrutura de um projeto e gera o script de deploy dele
#    (pergunta: nome, repo SSH, branch, binário do PHP)
./setup.sh            # gera ./deploy-<projeto>.sh
...
- `install.sh` — instala PHP (PPA ondrej, versão à escolha) + Composer, Node (nodesource), nginx e pnpm (corepack). Idempotente: pula o que já existe. Funciona como root ou com sudo.
- `setup.sh` — roda 1x por projeto: cria `/var/www/projects/<projeto>/{releases,shared/storage/...}` com permissões (`www-data`, setgid 2775), gera deploy key se faltar e monta o `deploy-<projeto>.sh` a partir do `deploy.template.sh` (placeholders `{{PROJECT}}`, `{{REPOSITORY}}`, `{{BRANCH}}`, `{{PHP_BINARY}}`).
